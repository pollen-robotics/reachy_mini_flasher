...
//!      it is retried for a couple of seconds, and a volume that never locks
//!      fails the flash - see `lock_and_dismount`),
...
//!   5. on drop: release the locks **and only then**
//!      `IOCTL_DISK_UPDATE_PROPERTIES`, so Explorer re-reads the freshly
//!      written partition table (a held lock blocks that re-read).
//!
...
/// AutoPlay can open its Explorer window a moment *after* the disk itself
/// enumerates, so the close is retried briefly rather than fired once and hoped
/// over. Cheap: each attempt that finds nothing costs one PowerShell run.
const AUTOPLAY_CLOSE_ATTEMPTS: u32 = 5;
const AUTOPLAY_CLOSE_DELAY: Duration = Duration::from_millis(800);
...
pub struct DiskLock {
    ...
    volumes: Vec<OwnedHandle>,
}

impl Drop for DiskLock {
    fn drop(&mut self) {
        // Release the locks FIRST, and explicitly.
        //
        // Rust runs a `Drop` body *before* dropping the struct's fields, so
        // leaving this to field order would fire IOCTL_DISK_UPDATE_PROPERTIES
        // while the volume handles are still open - and re-reading the disk
        // layout is precisely what a held lock prevents (the same reason
        // deleting the partition table voided our locks; see `lock_disk`). The
        // refresh would quietly do nothing and Explorer would keep showing the
        // pre-flash partitions until the drive was physically re-plugged.
        self.volumes.clear();

        // Only now nudge Windows into re-reading the partition table we just
        // wrote, so the robot's boot partition shows up instead of the stale one.
...
pub fn lock_disk(target: &str) -> Result<Option<DiskLock>, String> {
    ...
    Ok(Some(DiskLock { disk_number, volumes: locked }))
}
...
/// Close any Explorer window AutoPlay opened on the freshly exposed eMMC.
///
/// The moment rpiboot exposes the eMMC, Windows sees removable media appear and
/// AutoPlay pops an Explorer window on the robot's boot partition. Harmless in
/// itself - the flash locks and dismounts those volumes regardless - but it
/// covers the app in the middle of its own flow and reads as something having
/// gone wrong.
///
/// Deliberately narrow: only windows whose path is on *this* disk's drive
/// letters are closed, so a window the user opened somewhere else is left
/// alone. Nothing here changes a system setting - the user's AutoPlay
/// preferences are theirs, and an app has no business rewriting them.
///
/// Must run in the **non-elevated** GUI process: `Shell.Application` in an
/// elevated process belongs to a different integrity level and sees none of the
/// user's Explorer windows.
///
/// Best-effort throughout - it returns immediately and never fails the flow.
pub fn close_autoplay_windows(target: &str) {
    let Some(disk_number) = disk_number_from_target(target) else {
        return;
    };
    // On a worker thread: this waits out AutoPlay, and the caller (detection,
    // then the flash) has no reason to block on cosmetics.
    std::thread::spawn(move || {
        for _ in 0..AUTOPLAY_CLOSE_ATTEMPTS {
            match close_explorer_on_disk(disk_number) {
                Ok(0) => {}
                Ok(closed) => crate::flash::log(&format!(
                    "closed {closed} Explorer window(s) on PhysicalDrive{disk_number}"
                )),
                Err(e) => crate::flash::log(&format!("could not close Explorer windows: {e}")),
            }
            sleep(AUTOPLAY_CLOSE_DELAY);
        }
    });
}

/// Close Explorer windows showing a drive letter of `disk_number`; returns how
/// many were closed.
fn close_explorer_on_disk(disk_number: u32) -> Result<u32, String> {
    // `$shell.Windows()` also yields windows with no filesystem path at all
    // (Control Panel, and historically IE), which throw on `.Document.Folder` -
    // hence the per-window try/catch rather than a filter up front.
    let script = format!(
        "$letters = @(Get-Partition -DiskNumber {disk_number} -ErrorAction SilentlyContinue | \
           Where-Object {{ $_.DriveLetter }} | ForEach-Object {{ [string]$_.DriveLetter + ':' }}); \
         if (-not $letters) {{ '0'; exit 0 }}; \
         $closed = 0; \
         ...
         foreach ($w in @($shell.Windows())) {{ \
           try {{ \
             ...
             foreach ($l in $letters) {{ \
               if ($path -like ($l + '*')) {{ $w.Quit(); $closed++; break }} \
             }} \
           }} catch {{ }} \
         }}; \
         [string]$closed"
    );
    let out = crate::win_ps::run(&script, "close Explorer windows")?;
    let out = out.trim();
    out.parse()
        .map_err(|e| format!("unexpected output {out:?}: {e}"))
}
...
fn lock_and_dismount(handle: &OwnedHandle, volume: &str) -> Result<(), String> {
    ...
    // Dismount regardless of the lock: it tears down the mounted filesystem,
    // which is the part that would corrupt the write.
    let dismounted = ioctl_void(handle, FSCTL_DISMOUNT_VOLUME);

    match (locked, dismounted) {
        (true, Ok(())) => Ok(()),
        // A dismount without a lock is NOT good enough, though it used to be
        // accepted: the justification was that the disk's partition table was
        // deleted as well, leaving Windows nothing to auto-mount. That delete
        // is gone (it voided the locks - see `lock_disk`), and with neither in
        // place the volume can remount partway through the write. The
        // corruption that causes is silent, so fail now instead, with
        // something the user can act on.
        (false, _) => Err(format!(
            "Windows would not release {volume} (on the robot's storage): {last_err}. \
             Close any window browsing that drive, pause antivirus scanning of it, \
             then try again."
        )),
        (_, Err(e)) => Err(format!(
