            ...
            {effStatus === 'intro' ? (
              ...
              effConnectStep >= CONNECT_WAIT ? (
                <SelectReachyBody
                  ...
                  driverInstalling={installingDriver}
                />
              ) : effConnectStep === CONNECT_WARN ? (
                <SystemPromptsBody />
              ) : (
...
function SelectReachyBody({
  ...
  /** The install is running: a console window is on screen and will self-close. */
  driverInstalling?: boolean;
}) {
    ...
    <Stack spacing={2} sx={BODY_STACK_SX}>
      ...
      <Typography sx={{ ...DESC_SX, maxWidth: 400, minHeight: '3em' }}>
        {device ? (
          ...
          // The helper's own console is hidden (`-WindowStyle Hidden`, see
          // `win_ps::run_elevated`), so the only windows the user sees are
          // Windows' own: the UAC prompt and the driver-install dialog. Name
          // those rather than anything of ours, and say that no further input
          // is needed - otherwise a few seconds of apparent inactivity reads
          // as a hang and invites a click on Back.
          <>
            Windows is installing the <B>USB driver</B>. Approve the prompts as they appear -
            it takes a few seconds and needs nothing else from you.
          </>
        ) : driverNeeded ? (
          ...
          >
            ...
            <Typography sx={{ fontSize: '0.9375rem', color: 'text.secondary' }}>
              {driverInstalling
                ...
                : preparing
                  ? 'Reachy detected, preparing storage\u2026'
                  : 'Searching\u2026'}
            </Typography>
          </Stack>
        )}
...
function humanizeError(raw: string): { title: string; message: string } {
  ...
  // Windows: a volume on the target disk refused to be locked/dismounted. This
  // has to be tested BEFORE the authorization branch below: the underlying Win32
  // failure is ERROR_ACCESS_DENIED, so a looser match on 'denied' claims it
  // first and tells the user to approve a prompt they already approved.
  ...
  // Full phrases only, for the same reason: a bare 'denied' also matches
  // "Access is denied", which is a busy disk rather than an unanswered prompt.
  if (/authorization was denied|authorization denied|-128/.test(e)) {
    return {
